...
def trainAlgorithm(savepath,datapath,markersetpath,fs,num_epochs=2,prevModel=None,windowSize=120,
    ...
    if '.pickle' in datapath:
        ...
        data_segs = Subset(data_segs, indices=range(len(data_segs)))
        # Filter trajectories
...
def import_labelled_c3ds(filelist,markers,alignMkR,alignMkL,windowSize):
    ...
    for trial in filelist:
        ...
        alllabels = c3ddat['parameters']['POINT']['DESCRIPTIONS']['value']
...
def plotGraph(data, save=True):
    ...
    # Initialize scatters
    scatters = [ ax.scatter(data[i,:,0], data[i,:,1], data[i,:,2]) for i in range(data.shape[0]) ]
...
def animate_scatters(iteration, data, scatters):
    ...
    for i in range(data[0].shape[0]):
        scatters[i]._offsets3d = (data[i, :, 0], data[i, :, 1], data[i, :, 2])
    return scatters